...
!!! info "Common problems"
	...
	Another issue is the game not loading at all or having jumbled text, in this case, check Japanese locale is set and then restart your PC.
...
### Running Microsoft Windows XP in QEMU

QEMU (**Q**uick **EMU**lator) is a free and open source virtualizer and emulator that can perform hardware virtualization.

Requirements:

- A modern-ish Android, I remember doing this on a phone from 6 years ago, so any phone from recent times will be fine. 
- 4GB of free space on your device.
- Windows XP Disc Image. Ask me on Discord if you need help.
- Termux [(Google Play)](https://play.google.com/store/apps/details?id=com.termux)  
- VNC Viewer [(Google Play)](https://play.google.com/store/apps/details?id=com.realvnc.viewer.android)  
- Hacker's Keyboard [(Google Play)](https://play.google.com/store/apps/details?id=org.pocketworkstation.pckeyboard)  
...
After that is done we need to gain access to our Android filesystem.
...
#### Step 2. Create QEMU virtual hard disk

```bash
qemu-img create -f qcow2 WindowsXP.img 4G
```
!!! tip "Adjustable variable"
	You can adjust the capacity of the virtual hard disk by changing the value preceding *G*, e.g. 4G, 8G, 16G and so on.

#### Step 3. Start the virtual machine  

Now that our virtual hard disk is created, we can now launch the VM!

```bash
qemu-system-i386 -m 512 -smp cores=4,sockets=2 -vnc :0 -drive file=WindowsXP.img,format=qcow2 -cdrom <your_winxp_image>.iso -boot d
```
!!! note "Windows XP ISO File"
	Make sure you point the `-cdrom` argument to your Windows XP ISO. It is located for me at `/storage/emulated/0/Downloads/WXPVOL_EN.iso` so what you'd put in termux is `~/storage/downloads/WXPVOL_EN.iso`

#### Step 4. Connect to the display server

Open VNC Manager and tap the +, add the server `localhost:5900`, now connect.
