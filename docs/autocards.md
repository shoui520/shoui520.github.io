...
- mpv closes when I paste a hianime link — make sure your IP address isn't being blocked by hianime | make sure it's an **episode link** | make sure Windows is not blocking the included `yt-dlp`. Try updating yt-dlp using `.\yt-dlp.exe -U` in Terminal.   
